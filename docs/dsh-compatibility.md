...
Chief receives `meteor_init`, `meteor_start`, `meteor_status`, `meteor_control` and `meteor_evidence`. Its normal tools remain available. One start creates one research and one continuous native Agent; Meteor never spawns an automatic next research.
...
```text
dsh --profile headless --patch /absolute/path/to/meteor-test.patch.yml "Use meteor_start to run the research; collect its job and automatic integration jobs before your final response."
```
