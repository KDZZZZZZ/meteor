...
export interface SshProfile { ssh_alias: string; remote_root: string; queue_root?: string; driver_path?: string; device_id?: string; connect_timeout_seconds?: number; env_script?: string; npu_arch?: string }
...
export function loadSshProfile(ref: string, options: { profilesPath?: string } = {}): SshProfile {
  ...
  const allowed = ['ssh_alias','remote_root','queue_root','driver_path','device_id','connect_timeout_seconds','env_script','npu_arch'];
  ...
  if (profile.queue_root !== undefined) assert(typeof profile.queue_root === 'string' && /^\/[A-Za-z0-9_./-]+$/.test(profile.queue_root) && !profile.queue_root.split('/').includes('..') && !['/', '/tmp', '/home', '/mnt'].includes(profile.queue_root.replace(/\/+$/, '') || '/'), 'queue_root must be a dedicated absolute POSIX directory on a local filesystem');
